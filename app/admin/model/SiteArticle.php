...
use app\admin\validate\SiteArticle as ValidateModel ;
...
class SiteArticle extends Model{
	...
	//联查分类
	static public function getListPOAsInfo($where=[]){
		$m = new static ;
		return $m->alias('a')
					->field('a.*,b.title as cate_title')
					->leftJoin('tp_site_cate b','b.id = a.cate_id')
					->where($where)
					->paginate(10);
	...
	//单条
	static public function getOneAsInfo($id){
		$m = new static ;
		return $m->alias('a')
					->field('a.*,b.title as cate_title')
					->leftJoin('tp_site_cate b','b.id = a.cate_id')
