...
use app\admin\validate\SiteCate as ValidateModel ;
use think\exception\ValidateException;

class SiteCate extends Model{
	...
	static private function formatData($data){
		...
		if(empty($data['pid'])){
			...
			//上级
			$parent = static::getOne($data['pid']);
			$data['path'] = $parent['path'].'-'.$parent['id'] ;
			$data['lv'] = count(explode('-',$data['path']));
		}
		return $data ;
	...
	//根据path获取列表
	static public function getAllPath($where=null){
		$list = (new static) -> field('*,concat(path,"-",id) as bpath') -> where($where) -> order('bpath asc') -> select();
